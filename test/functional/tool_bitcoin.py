...
class ToolAureusTest(AureusTestFramework):
    def set_test_params(self):
        self.setup_clean_chain = True
        self.num_nodes = 1

    def skip_test_if_missing_module(self):
        # Skip test on windows because currently when `aureus node -version` is
        # run on windows, python doesn't capture output from the child
        # `aureusd` and `aureus-node` process started with _wexecvp, and
        # stdout/stderr are always empty. See
        # https://github.com/aureus/aureus/pull/33229#issuecomment-3265524908
    ...
    def set_cmd_args(self, node, args):
        """Set up node so it will be started through aureus wrapper command with specified arguments."""
        node.args = [self.binary_paths.aureus_bin] + args + ["node"] + self.node_options[node.index]
    ...
    def run_test(self):
        ...
        self.log.info("Ensure aureus node command invokes aureusd by default")
        self.test_args([], [], expect_exe="aureusd")

        self.log.info("Ensure aureus -M invokes aureusd")
        self.test_args(["-M"], [], expect_exe="aureusd")

        self.log.info("Ensure aureus -M does not accept -ipcbind")
        ...
        if self.is_ipc_compiled():
            self.log.info("Ensure aureus -m invokes aureus-node")
            self.test_args(["-m"], [], expect_exe="aureus-node")

            self.log.info("Ensure aureus -m does accept -ipcbind")
            self.test_args(["-m"], ["-ipcbind=unix"], expect_exe="aureus-node")

            self.log.info("Ensure aureus accepts -ipcbind by default")
            self.test_args([], ["-ipcbind=unix"], expect_exe="aureus-node")

            self.log.info("Ensure aureus recognizes -ipcbind in config file")
            append_config(node.datadir_path, ["ipcbind=unix"])
            self.test_args([], [], expect_exe="aureus-node")
