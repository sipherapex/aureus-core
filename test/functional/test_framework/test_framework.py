#!/usr/bin/env python3
# Copyright (c) 2014-present The Aureus Core developers
...
TMPDIR_PREFIX = "aureus_func_test_"
...
class AureusTestMetaClass(type):
    """Metaclass for AureusTestFramework.

    Ensures that any attempt to register a subclass of `AureusTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'AureusTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("AureusTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("AureusTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class AureusTestFramework(metaclass=AureusTestMetaClass):
    """Base class for a aureus test script.

    Individual aureus test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self, test_file):
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use aureus-cli instead of RPC for all commands")
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a aureusd test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple aureusd test nodes"""
        for node in self.nodes:
    ...
    def create_outpoints(self, node, *, outputs):
        """Send funds to a given list of `{address: amount}` targets using the aureusd
    ...
    def _initialize_chain(self):
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in aureus.conf
    ...
    def skip_if_no_aureusd_tracepoints(self):
        """Skip the running test if aureusd has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("aureusd has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_aureusd_zmq(self):
        """Skip the running test if aureusd has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("aureusd has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if aureus-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("aureus-wallet has not been compiled")

    def skip_if_no_aureus_tx(self):
        """Skip the running test if aureus-tx has not been compiled."""
        if not self.is_aureus_tx_compiled():
            raise SkipTest("aureus-tx has not been compiled")

    def skip_if_no_aureus_util(self):
        """Skip the running test if aureus-util has not been compiled."""
        if not self.is_aureus_util_compiled():
            raise SkipTest("aureus-util has not been compiled")

    def skip_if_no_aureus_chainstate(self):
        """Skip the running test if aureus-chainstate has not been compiled."""
        if not self.is_aureus_chainstate_compiled():
            raise SkipTest("aureus-chainstate has not been compiled")

    def skip_if_no_aureus_bench(self):
        """Skip the running test if bench_aureus has not been compiled."""
        if not self.is_bench_compiled():
            raise SkipTest("bench_aureus has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if aureus-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("aureus-cli has not been compiled.")
    ...
    def is_bench_compiled(self):
        """Checks whether bench_aureus was compiled."""
        return self.config["components"].getboolean("BUILD_BENCH")

    def is_cli_compiled(self):
        """Checks whether aureus-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether aureus-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_aureus_tx_compiled(self):
        """Checks whether aureus-tx was compiled."""
        return self.config["components"].getboolean("BUILD_BITCOIN_TX")

    def is_aureus_util_compiled(self):
        """Checks whether aureus-util was compiled."""
        return self.config["components"].getboolean("ENABLE_BITCOIN_UTIL")

    def is_aureus_chainstate_compiled(self):
        """Checks whether aureus-chainstate was compiled."""
