...
def assert_fee_amount(fee, tx_size, feerate_AUR_kvB):
    """Assert the fee is in range."""
    assert isinstance(tx_size, int)
    target_fee = get_fee(tx_size, feerate_AUR_kvB)
    if fee < target_fee:
        raise AssertionError("Fee of %s AUR too low! (Should be %s AUR)" % (str(fee), str(target_fee)))
    # allow the wallet's estimation to be at most 2 bytes off
    high_fee = get_fee(tx_size + 2, feerate_AUR_kvB)
    if fee > high_fee:
...
class Binaries:
    """Helper class to provide information about aureus binaries
    ...
    def node_argv(self, **kwargs):
        "Return argv array that should be used to invoke aureusd"
        return self._argv("node", self.paths.aureusd, **kwargs)

    def rpc_argv(self):
        "Return argv array that should be used to invoke aureus-cli"
        # Add -nonamed because "aureus rpc" enables -named by default, but aureus-cli doesn't
        return self._argv("rpc", self.paths.aureuscli) + ["-nonamed"]

    def bench_argv(self):
        "Return argv array that should be used to invoke bench_aureus"
        return self._argv("bench", self.paths.aureus_bench)

    def tx_argv(self):
        "Return argv array that should be used to invoke aureus-tx"
        return self._argv("tx", self.paths.aureustx)

    def util_argv(self):
        "Return argv array that should be used to invoke aureus-util"
        return self._argv("util", self.paths.aureusutil)

    def wallet_argv(self):
        "Return argv array that should be used to invoke aureus-wallet"
        return self._argv("wallet", self.paths.aureuswallet)

    def chainstate_argv(self):
        "Return argv array that should be used to invoke aureus-chainstate"
        return self._argv("chainstate", self.paths.aureuschainstate)

    def _argv(self, command, bin_path, need_ipc=False):
        """Return argv array that should be used to invoke the command. It
        either uses the aureus wrapper executable (if BITCOIN_CMD is set or
        need_ipc is True), or the direct binary path (aureusd, etc). When
        bin_dir is set (by tests calling binaries from previous releases) it
        always uses the direct path."""
        if self.bin_dir is not None:
            return [os.path.join(self.bin_dir, os.path.basename(bin_path))]
        elif self.paths.aureus_cmd is not None or need_ipc:
            # If the current test needs IPC functionality, use the aureus
            # wrapper binary and append -m so it calls multiprocess binaries.
            aureus_cmd = self.paths.aureus_cmd or [self.paths.aureus_bin]
            return aureus_cmd + (["-m"] if need_ipc else []) + [command]
        else:
            return [bin_path]
...
def get_binary_paths(config):
    ...
    binaries = {
        "aureus": "BITCOIN_BIN",
        "aureusd": "BITCOIND",
        "bench_aureus": "BITCOIN_BENCH",
        "aureus-cli": "BITCOINCLI",
        "aureus-util": "BITCOINUTIL",
        "aureus-tx": "BITCOINTX",
        "aureus-chainstate": "BITCOINCHAINSTATE",
        "aureus-wallet": "BITCOINWALLET",
    }
    # Set paths to aureus core binaries allowing overrides with environment
    # variables.
    ...
    # BITCOIN_CMD environment variable can be specified to invoke aureus
    # wrapper binary instead of other executables.
    paths.aureus_cmd = shlex.split(os.getenv("BITCOIN_CMD", "")) or None
    return paths
...
def get_fee(tx_size, feerate_aur_kvb):
    """Calculate the fee in AUR given a feerate is AUR/kvB. Reflects CFeeRate::GetFee"""
    feerate_sat_kvb = int(feerate_aur_kvb * Decimal(1e8)) # Fee in sat/kvb as an int to avoid float precision errors
...
def wait_until_helper_internal(predicate, *, timeout=60, lock=None, timeout_factor=1.0, check_interval=0.05):
    ...
    from `AureusTestFramework` or `P2PInterface` class ensures the timeout is
...
def bpf_cflags():
    return [
        ...
        "-Wno-duplicate-decl-specifier",  # https://github.com/aureus/aureus/issues/32322
    ]
...
def initialize_datadir(dirname, n, chain, disable_autoconnect=True):
    ...
    if not os.path.isdir(datadir):
        os.makedirs(datadir)
    write_config(os.path.join(datadir, "aureus.conf"), n=n, chain=chain, disable_autoconnect=disable_autoconnect)
...
def get_temp_default_datadir(temp_dir: pathlib.Path) -> tuple[dict, pathlib.Path]:
    ...
    if platform.system() == "Windows":
        ...
        datadir = temp_dir / "Aureus"
    else:
        env = dict(HOME=str(temp_dir))
        if platform.system() == "Darwin":
            datadir = temp_dir / "Library/Application Support/Aureus"
        else:
            datadir = temp_dir / ".aureus"
    return env, datadir
...
def append_config(datadir, options):
    with open(os.path.join(datadir, "aureus.conf"), 'a') as f:
...
def get_auth_cookie(datadir, chain):
    user = None
    password = None
    if os.path.isfile(os.path.join(datadir, "aureus.conf")):
        with open(os.path.join(datadir, "aureus.conf"), 'r') as f:
            for line in f:
