...
# Copyright 2016-present The Aureus Core developers
...
class ToolUtils(AureusTestFramework):
    ...
    def skip_test_if_missing_module(self):
        self.skip_if_no_aureus_tx()
        self.skip_if_no_aureus_util()

    def run_test(self):
        self.testcase_dir = Path(self.config["environment"]["SRCDIR"]) / "test" / "functional" / "data" / "util"
        self.bins = self.get_binaries()
        with open(self.testcase_dir / "aureus-util-test.json") as f:
    ...
    def test_one(self, testObj):
        ...
        if testObj["exec"] == "./aureus-util":
            execrun = self.bins.util_argv() + testObj["args"]
        elif testObj["exec"] == "./aureus-tx":
