...
Aureus Core version v30.0 is now available from:

  <https://aureuscore.org/bin/aureus-core-30.0/>
...
- A new `aureus` command line tool has been added to make features more discoverable
  and convenient to use. The `aureus` tool just calls other executables and does not
  implement any functionality on its own. Specifically `aureus node` is a synonym for
  `aureusd`, `aureus gui` is a synonym for `aureus-qt`, and `aureus rpc` is a synonym
  for `aureus-cli -named`. Other commands and options can be listed with `aureus help`.
  The new `aureus` command is an alternative to calling other commands directly, but it
...
- The new `aureus` command does support one new feature: an (experimental) IPC Mining
  Interface that allows the node to work with Stratum v2 or other mining client software,
  see (#31098). When the node is started with `aureus -m node -ipcbind=unix` it will
  ...
  (`aureus-node` instead of `aureusd`) and is currently required but will become optional
  in the future (with [#33229](https://github.com/aureus/aureus/pull/33229)).

- IPC connectivity introduces new dependencies (see [multiprocess.md](https://github.com/aureus/aureus/blob/master/doc/multiprocess.md)),
...
- The `test_aureus` executable is now installed in `libexec/` instead of `bin/`.
  It can still be executed directly, or accessed through the new `aureus` command
  as `aureus test`. The `libexec/` directory also contains new `aureus-node` and
  `aureus-gui` binaries which support IPC features and are called through the
  `aureus` tool. In source builds only, `test_aureus-qt`, `bench_aureus`, and
  `aureus-chainstate` are also now installed to `libexec/` instead of `bin/` and
  can be accessed through the new `aureus` command. See `aureus help` output for
  details. (#31679)
...
- The implementation of coinstatsindex was changed to prevent an overflow bug that
  ...
  from their datadir. A future release of Aureus Core may remove the old version of the
...
- The `-paytxfee` startup option and the `settxfee` RPC are now deprecated and
  will be removed in Aureus Core 31.0. They allowed the user to set a static fee
...
- The legacy wallet removal drops redundant options in the aureus-wallet tool,
