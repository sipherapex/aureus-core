# CJDNS support in Aureus Core

It is possible to run Aureus Core over CJDNS, an encrypted IPv6 network that
...
done outside of Aureus Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Aureus network and individual nodes.
...
before it will work with your Aureus Core node. This is described in steps
...
## Run Aureus Core with CJDNS

Once you are connected to the CJDNS network, the following Aureus Core
...
When enabled, this option tells Aureus Core that it is running in an
...
IPv6 local network. This helps Aureus Core perform better address management:
...
CJDNS support was added to Aureus Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `aureus-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/aureus/aureus/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Aureus Core

There are several ways to see your CJDNS address in Aureus Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `aureus-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `aureus-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `aureus-cli help getnodeaddresses` for details.
