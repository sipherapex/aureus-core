...
The options known to work for building Aureus Core on Windows are:
...
Note that for WSL the Aureus Core source path MUST be somewhere in the default mount file system, for
example /usr/src/aureus, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
```shell
cmake --install build --prefix /mnt/c/workspace/aureus
```
...
```shell
cmake --install build --prefix /mnt/c/workspace/aureus --strip
```
