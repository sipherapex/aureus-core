...
static void SetupAureusTxArgs(ArgsManager &argsman)
{
...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        std::string strUsage = CLIENT_NAME " aureus-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.GetBoolArg("-version", false)) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "The aureus-tx tool is used for creating and modifying aureus transactions.\n\n"
                "aureus-tx can be used with \"<hex-tx> [commands]\" to update a hex-encoded aureus transaction, or with \"-create [commands]\" to create a hex-encoded aureus transaction.\n"
                "\n"
                "Usage: aureus-tx [options] <hex-tx> [commands]\n"
                "or:    aureus-tx [options] -create [commands]\n"
                "\n";
