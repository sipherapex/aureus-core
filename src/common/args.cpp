...
const char * const BITCOIN_CONF_FILENAME = "aureus.conf";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //aureus-tx using stdin
...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\Aureus
    //   new: C:\Users\Username\AppData\Local\Aureus
    // macOS: ~/Library/Application Support/Aureus
    // Unix-like: ~/.aureus
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "Aureus";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "Aureus";
#else
    fs::path pathRet;
