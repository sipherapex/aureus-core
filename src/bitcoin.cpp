// Copyright (c) 2025-present The Aureus Core developers
...
Options:
  -m, --multiprocess     Run multiprocess binaries aureus-node, aureus-gui.
  -M, --monolithic       Run monolithic binaries aureusd, aureus-qt. (Default behavior)
  -v, --version          Show version information
  -h, --help             Show full help message

Commands:
  gui [ARGS]     Start GUI, equivalent to running 'aureus-qt [ARGS]' or 'aureus-gui [ARGS]'.
  node [ARGS]    Start node, equivalent to running 'aureusd [ARGS]' or 'aureus-node [ARGS]'.
  rpc [ARGS]     Call RPC method, equivalent to running 'aureus-cli -named [ARGS]'.
  wallet [ARGS]  Call wallet command, equivalent to running 'aureus-wallet [ARGS]'.
  tx [ARGS]      Manipulate hex-encoded transactions, equivalent to running 'aureus-tx [ARGS]'.
  help           Show full help message.
)";

static constexpr auto HELP_FULL = R"(
Additional less commonly used commands:
  bench [ARGS]      Run bench command, equivalent to running 'bench_aureus [ARGS]'.
  chainstate [ARGS] Run aureus kernel chainstate util, equivalent to running 'aureus-chainstate [ARGS]'.
  test [ARGS]       Run unit tests, equivalent to running 'test_aureus [ARGS]'.
  test-gui [ARGS]   Run GUI unit tests, equivalent to running 'test_aureus-qt [ARGS]'.
)";
...
int main(int argc, char* argv[])
{
    ...
    try {
        ...
        if (cmd.show_help || cmd.command.empty()) {
            ...
            args.emplace_back(UseMultiprocess(cmd) ? "aureus-gui" : "aureus-qt");
        } else if (cmd.command == "node") {
            args.emplace_back(UseMultiprocess(cmd) ? "aureus-node" : "aureusd");
        } else if (cmd.command == "rpc") {
            args.emplace_back("aureus-cli");
            // Since "aureus rpc" is a new interface that doesn't need to be
            ...
            args.emplace_back("aureus-wallet");
        } else if (cmd.command == "tx") {
            args.emplace_back("aureus-tx");
        } else if (cmd.command == "bench") {
            args.emplace_back("bench_aureus");
        } else if (cmd.command == "chainstate") {
            args.emplace_back("aureus-chainstate");
        } else if (cmd.command == "test") {
            args.emplace_back("test_aureus");
        } else if (cmd.command == "test-gui") {
            args.emplace_back("test_aureus-qt");
        } else if (cmd.command == "util") {
            args.emplace_back("aureus-util");
        } else {
...
//! Execute the specified aureusd, aureus-qt or other command line in `args`
...
//!             be a relative path to a aureusd, aureus-qt or other executable
...
//!
//!     strace -e trace=execve -s 10000 build/bin/aureus ...
...
static void ExecCommand(const std::vector<const char*>& args, std::string_view wrapper_argv0)
{
    ...
    // (https://github.com/aureus/aureus/pull/31375#discussion_r1861814807)
