...
Connection::~Connection()
{
    ...
    // object, which can run arbitrary blocking aureus code so they have to run
    ...
    // functions run, so client objects are fully disconnected before aureus
    // code in the destructors are run. This way if the aureus code tries to
...
void EventLoop::addAsyncCleanup(std::function<void()> fn)
{
    ...
    //
    // In aureus core, running these callbacks in the right order is
