...
//! Specify whether parameter should be parsed by aureus-cli as a JSON value,
 ...
 *
 * JSON parameters need to be listed here to make aureus-cli parse command line
 * arguments as JSON, instead of passing them as raw strings. `JSON` and
 * `JSON_OR_STRING` formats both make `aureus-cli` attempt to parse the
 ...
 * `aureus-cli getblockstats <hash>` without needing to quote the hash string
 * as JSON (`'"<hash>"'`).
 *
 ...
 * to make aureus-cli treat them as positional parameters when `-named` is used.
 * This prevents `aureus-cli` from splitting strings like "my=wallet" into a named
 ...
 * other string parameters for that method need to be listed as well so aureus-cli
 ...
 *
 * For example, the command `aureus-cli -named createwallet "my=wallet"`,
