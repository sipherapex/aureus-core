// Copyright (c) 2015-present The Aureus Core developers
...
void CompareBalance(WalletModel& walletModel, CAmount expected_balance, QLabel* balance_label_to_check)
{
    ...
    QString balanceComparison = AureusUnits::formatWithUnit(unit, expected_balance, false, AureusUnits::SeparatorStyle::ALWAYS);
...
//
//     QT_QPA_PLATFORM=xcb     build/bin/test_aureus-qt  # Linux
//     QT_QPA_PLATFORM=windows build/bin/test_aureus-qt  # Windows
//     QT_QPA_PLATFORM=cocoa   build/bin/test_aureus-qt  # macOS
void TestGUI(interfaces::Node& node, const std::shared_ptr<CWallet>& wallet)
{
    ...
    // Amount input
    AureusAmountField* amountInput = receiveCoinsDialog.findChild<AureusAmountField*>("reqAmount");
...
void WalletTests::walletTests()
{
    ...
    if (QApplication::platformName() == "minimal") {
        ...
        qWarning() << "Skipping WalletTests on mac build with 'minimal' platform set due to Qt bugs. To run AppTests, invoke "
                      "with 'QT_QPA_PLATFORM=cocoa test_aureus-qt' on mac, or else use a linux or windows build.";
        return;
    }
#endif
    TestGUI(m_node);
