...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaType<AureusUnit>("AureusUnit");
}
...
static const char* qt_argv = "aureus-qt";
...
void AureusApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the AureusApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", AureusGUI::DEFAULT_UIPLATFORM);
...
bool AureusApplication::createOptionsModel(bool resetSettings)
{
...
{
    window = new AureusGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &AureusGUI::quitRequested, this, &AureusApplication::requestShutdown);
...
void AureusApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool AureusApplication::baseInitialize()
{
...
void AureusApplication::startThread()
{
    ...
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &AureusApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &AureusApplication::handleRunawayException);
    connect(this, &AureusApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &AureusApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}
...
void AureusApplication::InitPruneSetting(int64_t prune_MiB)
{
...
void AureusApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if (paymentServer) {
        connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &AureusGUI::handlePaymentRequest);
        connect(window, &AureusGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void AureusApplication::handleNonFatalException(const QString& message)
{
...
bool AureusApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", AureusGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (invalid_token) {
            InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see aureus-qt -h for a list of options.", argv[i])));
            QMessageBox::critical(nullptr, CLIENT_NAME,
                                  // message cannot be translated because translations have not been initialized
                                  QString::fromStdString("Command line contains unexpected token '%1', see aureus-qt -h for a list of options.").arg(QString::fromStdString(argv[i])));
            return EXIT_FAILURE;
        }
    }
    ...
    /// 6-7. Parse aureus.conf, determine network, switch to network specific
