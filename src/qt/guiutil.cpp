...
#include <qt/aureusaddressvalidator.h>
...
bool parseAureusURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!AureusUnits::parse(AureusUnit::AUR, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
QString formatAureusURI(const SendCoinsRecipient &info)
{
    ...
    QString ret = QString("aureus:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(AureusUnits::format(AureusUnit::AUR, info.amount, false, AureusUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openAureusConf()
{
    ...
    /* Open aureus.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("Aureus (%s).lnk", ChainTypeToString(chain)));
}
...
fs::path static GetAutostartFilePath()
{
    ...
    return GetAutostartDir() / fs::u8path(strprintf("aureus-%s.desktop", ChainTypeToString(chain)));
}
