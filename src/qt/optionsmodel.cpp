...
/** Call node.updateRwSetting() with Aureus 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Aureus 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/aureus/aureus/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because aureus 22.x will not try to read these.
...
//! Convert enabled/size values to aureus -prune setting.
...
//! Get pruning enabled value to show in GUI from aureus -prune setting.
...
//! Get pruning size value to show in GUI from aureus -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayAureusUnit")) {
        settings.setValue("DisplayAureusUnit", QVariant::fromValue(AureusUnit::AUR));
    }
    QVariant unit = settings.value("DisplayAureusUnit");
    if (unit.canConvert<AureusUnit>()) {
        m_display_aureus_unit = unit.value<AureusUnit>();
    } else {
        m_display_aureus_unit = AureusUnit::AUR;
        settings.setValue("DisplayAureusUnit", QVariant::fromValue(m_display_aureus_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if aureus.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_aureus_unit);
...
{
    if (new_unit.isNull() || new_unit.value<AureusUnit>() == m_display_aureus_unit) return;
    m_display_aureus_unit = new_unit.value<AureusUnit>();
    QSettings settings;
    settings.setValue("DisplayAureusUnit", QVariant::fromValue(m_display_aureus_unit));
    Q_EMIT displayUnitChanged(m_display_aureus_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    // (https://github.com/aureus-core/gui/issues/567).
