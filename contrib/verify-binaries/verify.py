...
"""Script for verifying Aureus Core release binaries.

This script attempts to download the sum file SHA256SUMS and corresponding
signature file SHA256SUMS.asc from aureuscore.org and aureus.org and
compares them.
...
The builder keys are available in the guix.sigs repo:
    ...
    https://github.com/aureus-core/guix.sigs/tree/main/builder-keys
...
HOST1 = "https://aureuscore.org"
HOST2 = "https://aureus.org"
...
def verify_shasums_signature(
   ...
   ReturnCode, list[SigData], list[SigData], list[SigData], list[SigData]
    ...
    # which pubkeys within the Aureus community do we trust for the purposes of
    # binary verification?
    trusted_keys = set()
...
def verify_published_handler(args: argparse.Namespace) -> ReturnCode:
    WORKINGDIR = Path(tempfile.gettempdir()) / f"aureus_verify_binaries.{args.version}"
    ...
    # remove binaries that are known not to be hosted by aureuscore.org
...
def main():
    ...
    pub_parser.add_argument(
        'version', type=str, help=(
            f'version of the aureus release to download; of the format '
    ...
    pub_parser.add_argument(
        ...
        help=(
            ...
            '(Sometimes aureus.org lags behind aureuscore.org.)')
    )
