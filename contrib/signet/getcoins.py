#!/usr/bin/env python3
# Copyright (c) 2020-present The Aureus Core developers
...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing aureus-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='aureus-cli', help='aureus-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Aureus address to which the faucet should send')
...
parser.add_argument('aureus_cli_args', nargs='*', help='Arguments to pass on to aureus-cli (default: -signet)')

args = parser.parse_args()

if args.aureus_cli_args == []:
    args.aureus_cli_args = ['-signet']


def aureus_cli(rpc_command_and_params):
    argv = [args.cmd] + args.aureus_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = aureus_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = aureus_cli(['getnewaddress', 'faucet', 'bech32'])
