# Bootstrappable Aureus Core Builds

This directory contains the files necessary to perform bootstrappable Aureus
Core builds.
...
To build Aureus Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Aureus Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/aureus-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/aureus-core/guix.sigs).
