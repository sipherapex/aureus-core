...
be used in Aureus Core with the `-asmap` option.
...
to binary before being used with Aureus Core.
...
the `aureus-cli getnodeaddresses` command.
...
```
aureus-cli getnodeaddresses 0 > addrs.json
```
